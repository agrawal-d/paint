...
function gaussianBlur(x: number, y: number, input: Uint8ClampedArray, imgId: number) {
    ...
    function getIndex(x: number, y: number, layer: number) {
        return 4 * (y * BLUR_SIZE + x) + layer;
    }

    const getColor = (x: number, y: number, layer: number) => {
                ...
                for (let krnl_r = 0; krnl_r < KERNEL_SIZE; krnl_r++) {
                    for (let krnl_c = 0; krnl_c < KERNEL_SIZE; krnl_c++) {
                        output[getIndex(img_r, img_c, layer)] +=
                            ...
                            getColor(
                                img_r - (KERNEL_SIZE - 1) / 2 + krnl_r,
                                img_c - (KERNEL_SIZE - 1) / 2 + krnl_c,
                                layer,
                            );
                    }
                }
